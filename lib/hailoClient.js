...
class HailoClient {
	 ...
	 * @returns {{settings: object, info: object}} - Settings and info objects
	 */
	 ...
	 *
	 * @returns {Promise<{settings: object}|null>} - Settings object or null on error
	 */
	async readSettings() {
		try {
			await this.ensureAuth();
	 ...
	 *
	 * @param {{led?: number, pwr?: number, dist?: number}} settings - Settings to write
	 * @param {boolean} dryRun - If true, only log the command
	 */
	async writeSettings(settings = {}, dryRun = false) {
		try {
			...
			if (settings.led !== undefined) {
				params.append('led', String(settings.led));
			}
			if (settings.pwr !== undefined) {
				params.append('pwr', String(settings.pwr));
			}
			if (settings.dist !== undefined) {
				params.append('dist', String(settings.dist));
			}
