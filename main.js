...
class HailoLibero extends utils.Adapter {
	 ...
	 *
	 * @param {string} id - id of the state that has changed
		...
		try {
			switch (stateName) {
				...
				case 'settings.distance': {
					...
					const ledValue =
						stateName === 'settings.ledBrightness'
							? Number(state.val)
							: ledState?.val
								? Number(ledState.val)
								: 5;
					const forceValue =
						stateName === 'settings.ejectionForce'
							? Number(state.val)
							: forceState?.val
								? Number(forceState.val)
								: 5;
					const distanceValue =
						stateName === 'settings.distance'
							? Number(state.val)
							: distanceState?.val
								? Number(distanceState.val)
								: 50;
